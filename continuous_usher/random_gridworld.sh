...
offset=".11"
clip=0.3
args='--entropy-regularization=0.001 --n-test-rollouts=50 --n-cycles=500 --n-batches=4'
...
for env_name in "RandomGridworld" "StandardCarRandomGridworld" 
...
do
	...
	if [[ $env_name == "StandardCarRandomGridworld" ]]; then
		epochs=50
	fi
	if [[ $env_name == "StandardCarRandomBlockyGridworld" ]]; then
		epochs=50
	fi
