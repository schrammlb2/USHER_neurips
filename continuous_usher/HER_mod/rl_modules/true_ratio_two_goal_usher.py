...
from HER_mod.rl_modules.models import ratio_critic as critic
...
class ValueEstimator:
  ...
  def q_loss(self, preprocced_tuple, actor, transitions):
    ...
    with torch.no_grad():        
        ...
        t_remaining_tensor = torch.tensor(transitions['t_remaining'], dtype=torch.float32) 
        ...
        q_next_value, p_next_value = self.critic_target_1(inputs_next_norm_tensor_pol, actions_next, return_p=True)
        ...
        if low_range: 
            ...
            # target_p_value = exact_goal_tensor/t_remaining_tensor + p_next_value*(t_remaining_tensor-1)/t_remaining_tensor
            # target_p_value = r_tensor + self.args.gamma * p_next_value * (-r_tensor) + 1
            # target_p_value = (r_tensor + 1)/t_remaining_tensor + p_next_value*(t_remaining_tensor-1)/t_remaining_tensor
            # target_p_value = exact_goal_tensor + self.args.gamma * p_next_value * (1-exact_goal_tensor)
            target_p_value = target_p_value.detach()
            target_p_value = torch.clamp(target_p_value, 0, 1)
        # target_p_value = torch.clamp(target_p_value, 0, 1)

        
    q0, p0 = self.critic_1(inputs_norm_tensor_pol, actions_tensor, return_p=True)
