  ...
  def q_loss(self, preprocced_tuple, actor, transitions):
    ...
    if self.args.apply_ratio: 
        ...
        if method == "fuzz": 
            ...
            c = .01
            def indep_w(alpha):  
                return alpha*(p_fuzz.detach() + c)/(alpha*p_fuzz.detach() + (1-alpha)*p_fuzz_next.detach() + c)*her_used + (1-her_used)

            def her_w(alpha):  
                return (1-alpha)*(p0.detach() + c)/(alpha*p0.detach() + (1-alpha)*p_next_value.detach() + c)*her_used + (1-her_used)
            ...
            # p_loss += (her_used*((p_fuzz).pow(2))).mean()# - 2*(realized_p/t).mean()/100
