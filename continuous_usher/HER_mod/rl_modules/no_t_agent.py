...
class ValueEstimator:
  ...
  def q_loss(self, preprocced_tuple, actor, transitions):
    ...
    with torch.no_grad():        
        ...
        _, p_next_value = self.critic_target_1(inputs_next_norm_tensor_pol, map_t(t-1), actions_next, return_p=True)
    ...
    _, p0 = self.critic_1(inputs_norm_tensor_pol, map_t(t), actions_tensor, return_p=True)
    ...
    if self.args.apply_ratio: 
        ...
        if method == "fuzz": 
            ...
            _, p_fuzz = self.critic_1(fuzz_input, map_t(t), actions_tensor, return_p=True)
            _, p_fuzz_next = self.critic_target_1(fuzz_input_next, map_t(t), actions_tensor, return_p=True)
...
class ddpg_agent:
    def __init__(self, args, env, env_params, vel_goal=True):
        ...
        key = f"name_{args.env_name}__noise_{args.action_noise}.txt"
