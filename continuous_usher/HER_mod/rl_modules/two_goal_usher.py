...
class ValueEstimator:
  def __init__(self, env_params, args, agent):
    self.agent = agent
    self.args = args
  ...
  def get_value(self, obs, g, actor):
    with torch.no_grad():
        input_tensor = self.agent._preproc_inputs(obs, g)
        pi = actor(input_tensor, deterministic=True)
        actions_tensor = pi.detach().cpu()
        actions = actions_tensor.numpy().squeeze(axis=0)
        input_tensor_pol = self.agent._preproc_inputs(obs, g, gpi=g)
        # value = self.critic.min_critic(input_tensor_pol, actions_tensor).mean().numpy().squeeze()
        return self.critic_target_1(input_tensor_pol, actions_tensor).mean().numpy().squeeze()

        # print_fraction = 1/(self.args.n_test_rollouts*100)
  ...
  def q_loss(self, preprocced_tuple, actor, transitions):
    ...
    with torch.no_grad():        
        ...
        # if np.random.rand() < 1/print_fraction: 
        #     # pdb.set_trace()
        #     value = self.get_value(torch.tensor(transitions['obs'], dtype=torch.float32).squeeze(),
        #                             torch.tensor(transitions['policy_g'], dtype=torch.float32).squeeze(), 
        #                             actor).mean()
        #     print(f"value: {value}")
            # print(f"target_q_value: {target_q_value.mean()}, \t q_next_value: {q_next_value.mean()}") 
        # clip the q value
    ...
    if self.args.apply_ratio: 
        p0 = q1.detach()/clip_return + 1
        p1 =  target_q_value/clip_return + 1
        c = .1
...
class ddpg_agent:
    def __init__(self, args, env, env_params, vel_goal=True):
        ...
        self.critic = ValueEstimator(env_params, args, self)
    ...
    def _preproc_inputs(self, obs, g, gpi=None):
        ...
        if gpi is not None: 
            ...
            inputs = np.concatenate([obs_norm, g_norm, gpi_norm], axis=-1)
        else:
    ...
    def _eval_agent(self):
        ...
        for _ in range(self.args.n_test_rollouts):
            ...
            #     # value = self.critic.min_critic(input_tensor_pol, actions_tensor).mean().numpy().squeeze()
            #     value = self.critic.critic_target_1(input_tensor_pol, actions_tensor).mean().numpy().squeeze()

            #     # print_fraction = 1/(self.args.n_test_rollouts*100)
            #     # if np.random.rand() < 1/print_fraction: 
            #     #     print(f"value: {value.mean()}") 
            #     total_value += value
            total_value += self.critic.get_value(obs, g, self.actor_network)
