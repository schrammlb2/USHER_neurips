...
class ContinuousAcrobotEnv(core.Env):
    ...
    **Gif**: two blue pendulum links connected by two green joints. The joint in between the two pendulum links is acted
    ...
    def step(self, a):
        ...
        # return (self._get_ob(self.state), reward, terminal, {})
    ...
    def get_observation(self):
    	return {
    		"observation": self._get_ob(self.state),
    		"achieved_goal": self.state_to_goal(self.state),
    ...
    def state_to_goal(self, s):
    	return np.array(
            [cos(s[0]), sin(s[0]), cos(s[1]), sin(s[1])], dtype=np.float32
        )

    def compute_reward(self, a, b, info=None):
        s = a
        return (-cos(s[...,0]) - cos(s[...,1] + s[...,0]) > 1.0)*1-1

    	# tolerance = .25
    	# is_close = ((a - b)**2).sum(axis=-1)**.5 < tolerance
    	# return is_close*1 - 1
    	# return ((cos(a[0] - b[0]) > 1-tolerance) and (cos(a[1] - b[1]) > 1-tolerance))*1 - 1
