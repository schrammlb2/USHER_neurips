...
StateTensor = NewType('StateTensor', Tensor)
ActionTensor = NewType('ActionTensor', Tensor)
GoalTensor = NewType('GoalTensor', Tensor)
NormalStateTensor = NewType('NormalStateTensor', NormedTensor)
NormalActionTensor = NewType('NormalActionTensor', NormedTensor)
NormalGoalTensor = NewType('NormalGoalTensor', NormedTensor)


Array = NewType('Array', np.ndrray)
NormedArray = NewType('NormedArray', np.ndrray)
StateArray = NewType('StateArray', Array)
ActionArray = NewType('ActionArray', Array)
GoalArray = NewType('GoalArray', Array)
NormalStateArray = NewType('NormalStateArray', NormedArray)
NormalActionArray = NewType('NormalActionArray', NormedArray)
NormalGoalArray = NewType('NormalGoalArray', NormedArray)
  ...
  def q_loss(self, preprocced_tuple, actor, transitions):
    ...
    with torch.no_grad():        
        ...
        if low_range: 
            ...
            # target_p_value = (r_tensor + 1)/t + p_next_value*(t-1)/t
