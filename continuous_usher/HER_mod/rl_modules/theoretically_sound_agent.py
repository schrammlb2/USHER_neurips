...
from HER_mod.rl_modules.models import sac_actor as actor, critic as q_critic
  ...
  def q_loss(self, preprocced_tuple, actor, transitions):
    ...
    if self.args.apply_ratio: 
        ...
        if method == "fuzz": 
            ...
            alpha = .5
            p_num = p0.detach()
            p_denom =  alpha*p_next_value.detach() + (1-alpha)*p_num
            c = .01
            true_ratio = alpha*(p_num+c)/(p_denom+c)
            ...
            # critic_loss  = (p_ratio*(target_q_value - q0).pow(2)).mean()
            # critic_loss += (true_fuzz_ratio*(target_q_fuzz_value - q_fuzz).pow(2)).mean()
            # critic_loss += (p_ratio*(((p0).pow(2)  - (t-1)/t*(p0*target_p_value))*her_used)).mean()
            # critic_loss += (true_fuzz_ratio*her_used*((p_fuzz).pow(2)- (t-1)/t*p_fuzz*p_fuzz_next)).mean() - 2*(realized_p/t).mean()
            ...
            c = 1/(self.args.replay_k + 1)
            p_num = p0.detach()
            p_denom =  p_next_value.detach()*c + p_num*(1-c)

            true_ratio = p_num/p_denom
            clip_scale = 1+self.args.ratio_clip#1.4
            true_ratio = torch.clamp(true_ratio, 1/clip_scale, clip_scale)
            true_ratio = true_ratio*her_used + (1-her_used)

            critic_loss = (true_ratio*((target_q_value - q0).pow(2).mean() + ((p0).pow(2).mean()  - realized_p.mean()/t - (t-1)/t*(p0*target_p_value))*her_used))

        else: 
            assert False, "method not selected"
...
class ddpg_agent:
    def __init__(self, args, env, env_params, vel_goal=True):
        ...
        key = f"name_{args.env_name}__noise_{args.action_noise}__agent_usher.txt"
    ...
    def _update_network(self):
        ...
        inputs_norm_tensor, inputs_norm_tensor_pol = self.get_input_tensor(o, g, pol_g)
        inputs_next_norm_tensor, inputs_next_norm_tensor_pol = self.get_input_tensor(o_next, g, pol_g)
        _, duplicated_g_input = self.get_input_tensor(o, g, g)
