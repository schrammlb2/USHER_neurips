...
class AltGridworldEnv(GoalEnv):
	...
	def get_obs(self):
		...
		def type_to_failchance(blocktype):
			if blocktype == BLOCK: 
				chance = 1
		...
		def fail_chance(move):
			...
			return type_to_failchance(blocktype)


		# surroundings  = [fail_chance(move) == BLOCK for move in moves]
		# surroundings  = [s/len(surroundings) for s in surroundings]
		# surroundings = []
		surroundings = get_raytraces(ag, self.grid)
		map_grid = self.grid[1:-1, 1:-1].flatten()
		ttf = np.vectorize(type_to_failchance)
