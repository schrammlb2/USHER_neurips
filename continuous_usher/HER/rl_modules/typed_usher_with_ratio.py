...
from HER.mpi_utils.mpi_utils import sync_networks, sync_grads #type: ignore
from HER.mpi_utils.normalizer import normalizer #type: ignore
from HER.rl_modules.replay_buffer import replay_buffer #type: ignore
# from HER.rl_modules.usher_models import actor, critic, dual_critic #type: ignore
# from HER.rl_modules.usher_models import critic, dual_critic #type: ignore
from HER.rl_modules.usher_models import test_T_conditioned_ratio_critic as critic #type: ignore
from HER.rl_modules.alternate_sac_actor import actor #type: ignore
from HER.her_modules.usher import her_sampler #type: ignore
import pdb
import math #type: ignore
from typing import *
# from her_types import ObsArray
...
class ddpg_agent:
    def __init__(self, args: argparse.Namespace, env, env_params):
        self.args = args
        self.env = env
    ...
    def learn(self) -> None:
        ...
        for epoch in range(self.args.n_epochs):
            ...
            for _ in range(self.args.n_cycles):
                ...
                mb_obs_arr = np.array(mb_obs)
                mb_ag_arr = np.array(mb_ag)
                mb_g_arr = np.array(mb_g)
                mb_actions_arr = np.array(mb_actions)
                # store the episodes
                self.buffer.store_episode([mb_obs_arr, mb_ag_arr, mb_g_arr, mb_actions_arr])
                self._update_normalizer([mb_obs_arr, mb_ag_arr, mb_g_arr, mb_actions_arr])
    ...
    def _preproc_inputs(self, obs: ObsArray, g: GoalArray, gpi: Optional[GoalArray] = None) -> NormedTensor:
        obs_norm = self.o_norm.normalize(obs)
        g_norm = self.g_norm.normalize(g)
        # concatenate the stuffs
        gpi_norm = [] if type(gpi) == type(None) else [self.g_norm.normalize(gpi)]
        npinputs = np.concatenate([obs_norm, g_norm] + gpi_norm)#, g_norm])
        inputs = torch.tensor(npinputs, dtype=torch.float32).unsqueeze(0)
        if self.args.cuda:
            inputs = inputs.cuda()
        return NormedTensor(inputs)
    ...
    def _preproc_og(self, o: ObsArray, g: GoalArray) -> Tuple[ObsArray, GoalArray]:
        clipped_o = ObsArray(np.clip(o, -self.args.clip_obs, self.args.clip_obs))
        clipped_g = GoalArray(np.clip(g, -self.args.clip_obs, self.args.clip_obs))
        return clipped_o, clipped_g
