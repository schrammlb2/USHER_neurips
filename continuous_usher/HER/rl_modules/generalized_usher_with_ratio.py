...
from HER.rl_modules.usher_models import T_conditioned_ratio_critic as critic
...
class ddpg_agent:
    ...
    def _update_network(self):
        ...
        with torch.no_grad():
            ...
            # q_next_value, p_next_value = self.critic_target_network(inputs_next_norm_tensor, map_t(t-1), actions_next, return_p=True)
            q_next_value, p_next_value = self.critic_target_network(inputs_next_norm_tensor, torch.zeros_like(t), actions_next, return_p=True)
        ...
        # the q loss
        q0, p0 = self.critic_network(inputs_norm_tensor, torch.zeros_like(t), actions_tensor, return_p=True)
        low_range = False
        if low_range: 
        ...
        if self.args.apply_ratio: 
            ...
            true_ratio = torch.ones_like(true_ratio)
            true_ratio.detach()
            assert (true_ratio*((target_q_value - q0).pow(2))).shape == (((target_q_value - q0).pow(2))).shape
            assert torch.isclose((true_ratio*((target_q_value - q0).pow(2))), (((target_q_value - q0).pow(2)))).all()
            # max_val = .01
            # torch.clamp(true_ratio, 1-max_val, 1+max_val)
            true_ratio = 1
            # pdb.set_trace()
    ...
    def _eval_agent(self):
        ...
        for _ in range(self.args.n_test_rollouts):
            ...
            with torch.no_grad():
                ...
                t = torch.tensor([[1]])
                value = self.critic_network(input_tensor_pol, t, actions_tensor).mean().numpy().squeeze()
                total_value += value
