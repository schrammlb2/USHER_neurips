...
class ValueEstimator:
  ...
  def _soft_update_target_network(self, target, source):
    for target_param, param in zip(target.parameters(), source.parameters()):
        target_param.data.copy_((1 - self.polyak_base) * param.data + (self.polyak_base) * target_param.data)
    ...
    def _eval_agent(self, final=False):
        ...
        for _ in range(self.args.n_test_rollouts*test_num_multiplier):
            ...
            with torch.no_grad():
                ...
                value = self.critic.min_critic(inputs_norm_tensor, torch.tensor([[1]]), actions_tensor).mean().numpy().squeeze()
                total_value += value
